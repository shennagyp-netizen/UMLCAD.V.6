...
fn classify_planar_line_surface(
    ...
    if denominator.abs() <= parallel_bound {
        ...
        return Ok(Some(LineSurfaceIntersectionResult {
    ...
    if alpha < -param_tol_u
        || alpha > 1.0 + param_tol_u
        || beta < -param_tol_v
        || beta > 1.0 + param_tol_v
    {
        return Ok(Some(LineSurfaceIntersectionResult {
            status: IntersectionStatus::NoIntersection,
            points: Vec::new(),
...
mod tests {
    ...
    fn boundary_corner_intersection_is_unique() {
        let line = LineSegment3D {
            start: Point3 { x: -1.0, y: -1.0, z: -1.0 },
            end: Point3 { x: 0.0, y: 0.0, z: 0.0 },
        };
    ...
    fn coplanar_line_is_underdetermined() {
        let line = LineSegment3D {
            start: Point3 { x: -1.0, y: 0.5, z: 0.0 },
            end: Point3 { x: 4.0, y: 0.5, z: 0.0 },
        };
        assert_eq!(
            classify_planar_line_surface(line, &plane_with_domain(), 1e-10),
            Err(LineSurfaceIntersectionError::CoincidentOrUnderdetermined)
        );
    }
}
