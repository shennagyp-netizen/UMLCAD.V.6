...
) -> Result<Patch, SurfaceSurfaceIntersectionError> {
    surface
        .validate()
...
fn constrain(
    ...
    if slope.abs() <= 1e-14 {
        return base >= min - tol && base <= max + tol;
    }
    let t0 = (min - base) / slope;
    let t1 = (max - base) / slope;
    *lo = (*lo).max(t0.min(t1));
    *hi = (*hi).min(t0.max(t1));
    *lo <= *hi + tol
}
...
pub fn intersect_planar_nurbs_surfaces(
    ...
    let d2 = dot(dir, dir);
    if !d2.is_finite()
        || d2 <= 1e-24 * (norm(n1).max(norm(n2)).max(1.0)).powi(2)
    {
        return Err(SurfaceSurfaceIntersectionError::ParallelOrCoincident);
    }
    let h1 = dot(n1, a.origin);
    let h2 = dot(n2, b.origin);
    let base = scale(
        cross(
            sub(scale(n2, h1), scale(n1, h2)),
            dir,
        ),
        1.0 / d2,
    );
    let direction = scale(dir, 1.0 / norm(dir));
    let uva = uv_at(a, base)?;
    let uva1 = uv_at(a, add(base, direction))?;
    let uvb = uv_at(b, base)?;
    let uvb1 = uv_at(b, add(base, direction))?;
    ...
    let p0 = add(base, scale(direction, lo));
    let p1 = add(base, scale(direction, hi));
...
mod tests {
    ...
    fn transverse_patches_have_bounded_intersection() {
        let r = intersect_planar_nurbs_surfaces(&horizontal(), &vertical(), 1e-10).unwrap();
    ...
    #[test]
    fn parallel_patches_are_explicitly_underdetermined() {
        ...
        assert_eq!(
            ...
            Err(SurfaceSurfaceIntersectionError::ParallelOrCoincident)
        );
    }
}
