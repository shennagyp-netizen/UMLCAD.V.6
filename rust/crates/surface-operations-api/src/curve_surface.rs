...
pub fn intersect_nurbs_curve_surface(curve: &NurbsCurve3DDefinition, surface: &NurbsSurface3DDefinition, tolerance: f64) -> Result<CurveSurfaceIntersectionResult, CurveSurfaceIntersectionError> {
    ...
    let scale=curve.control_points.iter().map(|p| p.x.hypot(p.y.hypot(p.z))).fold(1.0,f64::max).max(surface.control_points.iter().map(|p| p.norm()).fold(1.0,f64::max)); let residual=tolerance.max(1e-10*scale);
    let mut roots=Vec::new(); let seeds=5usize;
    ...
    roots.sort_by(|a,b|a.curve_parameter.total_cmp(&b.curve_parameter).then(a.u.total_cmp(&b.u)).then(a.v.total_cmp(&b.v))); let status=match roots.len(){0=>IntersectionStatus::NoIntersection,1=>IntersectionStatus::Unique,_=>IntersectionStatus::Ambiguous}; Ok(CurveSurfaceIntersectionResult{status,points:roots})
}

pub trait NurbsCurveSurfaceOperations { fn intersect_nurbs_curve_surface(&self, curve:&NurbsCurve3DDefinition, surface:&NurbsSurface3DDefinition, tolerance:f64)->Result<CurveSurfaceIntersectionResult,CurveSurfaceIntersectionError>; }
...
fn line()->NurbsCurve3DDefinition{NurbsCurve3DDefinition::new(1,vec![CurvePoint3{x:0.25,y:0.75,z:-1.0},CurvePoint3{x:0.25,y:0.75,z:1.0}],vec![1.0,1.0],vec![0.0,0.0,1.0,1.0])}
#[test]fn unique_linear_curve_surface_root(){let r=intersect_nurbs_curve_surface(&line(),&plane(),1e-10).unwrap();assert_eq!(r.status,IntersectionStatus::Unique);assert_eq!(r.points.len(),1);assert!((r.points[0].curve_parameter-0.5).abs()<1e-10);assert!((r.points[0].u-0.25).abs()<1e-10);assert!((r.points[0].v-0.75).abs()<1e-10);assert!(r.points[0].point.z.abs()<1e-10);}
#[test]fn curve_parameter_domain_is_preserved(){let mut c=line();c.knots=vec![3.0,3.0,7.0,7.0];let r=intersect_nurbs_curve_surface(&c,&plane(),1e-10).unwrap();assert_eq!(r.status,IntersectionStatus::Unique);assert!((r.points[0].curve_parameter-5.0).abs()<1e-10);}
}