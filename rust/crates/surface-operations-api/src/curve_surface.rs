...
pub enum CurveSurfaceIntersectionError {
    NonFinite,
    InvalidCurve,
    InvalidSurface,
    NumericalFailure,
...
pub struct CurveSurfaceIntersectionPoint {
...
pub struct CurveSurfaceIntersectionResult {
    ...
    pub points: Vec<CurveSurfaceIntersectionPoint>,
}

fn basis(i: usize, p: usize, t: f64, knots: &[f64]) -> f64 {
    if p == 0 {
        let last = knots.len() - 1;
        return if (knots[i] <= t && t < knots[i + 1]) || (t == knots[last] && i + 1 == last) {
            1.0
        } else {
            0.0
        };
...
fn evaluate_curve(
    ...
    let mut h0 = CurvePoint3 { x: 0.0, y: 0.0, z: 0.0 };
    let mut h1 = h0;
    let mut w0 = 0.0;
    let mut w1 = 0.0;
    for i in 0..curve.control_points.len() {
        let w = curve.weights[i];
        let b0 = basis(i, curve.degree, t, &curve.knots);
        let b1 = basis_derivative(i, curve.degree, t, &curve.knots);
        let s0 = b0 * w;
        let s1 = b1 * w;
...
fn distance(a: Point3, b: CurvePoint3) -> f64 {
    (a.x - b.x).hypot((a.y - b.y).hypot(a.z - b.z))
}

fn point_norm(a: CurvePoint3) -> f64 {
    a.x.hypot(a.y.hypot(a.z))
}

fn dot_curve(a: CurvePoint3, b: Point3) -> f64 {
...
fn isolated_root_is_tangent(
    ...
    let scale = tangent_norm.max(normal_norm).max(1.0);
    let threshold = tolerance.max(1e-10) * tangent_norm * normal_norm * 10.0 / scale;
    tangent_norm > 1e-12 * scale
        && normal_norm > 1e-12 * scale
...
pub fn intersect_nurbs_curve_surface(
    ...
    for it in 0..seeds {
        for iu in 0..seeds {
            for iv in 0..seeds {
                let mut t = t0 + (t1 - t0) * (it as f64 + 0.5) / seeds as f64;
...
mod tests {
    ...
    fn tangent_curve() -> NurbsCurve3DDefinition {
        NurbsCurve3DDefinition::new(
            2,
            vec![
                CurvePoint3 { x: -1.0, y: 0.0, z: 1.0 },
                CurvePoint3 { x: 0.0, y: 0.0, z: -1.0 },
                CurvePoint3 { x: 1.0, y: 0.0, z: 1.0 },
            ],
            vec![1.0; 3],
    ...
    fn unique_linear_curve_surface_root() {
        ...
        assert!((r.points[0].curve_parameter - 0.5).abs() < 1e-10);
        assert!((r.points[0].u - 0.25).abs() < 1e-10);
        assert!((r.points[0].v - 0.75).abs() < 1e-10);
        assert!(r.points[0].point.z.abs() < 1e-10);
    }

    #[test]
    fn curve_parameter_domain_is_preserved() {
        let mut c = line();
        c.knots = vec![3.0, 3.0, 7.0, 7.0];
        let r = intersect_nurbs_curve_surface(&c, &plane(), 1e-10).unwrap();
        assert_eq!(r.status, IntersectionStatus::Unique);
        assert!((r.points[0].curve_parameter - 5.0).abs() < 1e-10);
    }

    #[test]
    fn isolated_quadratic_tangent_is_explicitly_reported() {
        assert_eq!(
