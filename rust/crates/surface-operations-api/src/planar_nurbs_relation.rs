...
pub fn classify_planar_nurbs_surface_relation(
    ...
    tolerance: f64,
) -> Result<PlanarNurbsSurfaceRelation, ()> {
    if !tolerance.is_finite() || tolerance < 0.0 { return Err(()); }
    surface.validate().map_err(|_| ())?;
    if !plane.normal.norm().is_finite() || plane.normal.norm() == 0.0 { return Err(()); }
    if [plane.origin.x, plane.origin.y, plane.origin.z].iter().any(|v| !v.is_finite()) { return Err(()); }

    let distances: Vec<f64> = surface.control_points.iter().map(|p| plane.signed_distance(*p)).collect();
    if distances.iter().any(|d| !d.is_finite()) { return Err(()); }
    let scale = surface.control_points.iter().map(|p| p.norm()).fold(1.0, f64::max)
...
mod tests {
    ...
    fn plane() -> Plane3D {
        Plane3D { origin: Point3 { x: 0.0, y: 0.0, z: 0.0 }, normal: Point3 { x: 0.0, y: 0.0, z: 1.0 } }
    }
    ...
    fn invalid_plane_is_rejected() {
        assert!(classify_planar_nurbs_surface_relation(
            Plane3D { origin: Point3 { x: 0.0, y: 0.0, z: 0.0 }, normal: Point3 { x: 0.0, y: 0.0, z: 0.0 } },
            &patch(1.0),
            1e-10,
        ).is_err());
    }
