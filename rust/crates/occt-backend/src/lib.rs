...
impl Clone for OcctShape { fn clone(&self)->Self{let mut output=std::ptr::null_mut();let status=unsafe{umlcad_occt_shape_clone(self.raw.as_ptr(),&mut output)};assert_eq!(status,OCCT_OK,"OCCT shape clone failed with status {status}");Self{raw:NonNull::new(output).expect("OCCT clone returned a null shape")}} }
impl Drop for OcctShape { fn drop(&mut self){unsafe{umlcad_occt_shape_delete(self.raw.as_ptr())}} }

pub struct OcctBackend;
impl OcctBackend {
    pub const fn new()->Self{Self}
    fn validate_positive_feature(value:f64,modeling_tolerance:f64,label:&'static str)->Result<(),GeometryError>{if !value.is_finite()||value<=0.0{return Err(GeometryError::InvalidInput(label));}if value<=OCCT_REFERENCE_MIN_FEATURE.max(modeling_tolerance){return Err(GeometryError::InvalidInput("geometry feature is below the reference backend resolution"));}Ok(())}
    fn validate_dimensions(w:f64,d:f64,h:f64,tol:f64)->Result<(),GeometryError>{Self::validate_positive_feature(w,tol,"box dimensions must be finite and positive")?;Self::validate_positive_feature(d,tol,"box dimensions must be finite and positive")?;Self::validate_positive_feature(h,tol,"box dimensions must be finite and positive")}
    fn validate_cylinder(r:f64,h:f64,tol:f64)->Result<(),GeometryError>{Self::validate_positive_feature(r,tol,"cylinder radius must be finite and positive")?;Self::validate_positive_feature(h,tol,"cylinder height must be finite and positive")}
    ...
    fn validate_cone(br:f64,tr:f64,h:f64,tol:f64)->Result<(),GeometryError>{Self::validate_positive_feature(br,tol,"cone base radius must be finite and positive")?;Self::validate_positive_feature(tr,tol,"cone top radius must be finite and positive")?;Self::validate_positive_feature(h,tol,"cone height must be finite and positive")}
...
impl GeometryBackend for OcctBackend {
    ...
    fn cone_solid(&self,br:f64,tr:f64,h:f64,t:ToleranceContext)->Result<GeometryResult<Self::Shape>,GeometryError>{t.validate()?;Self::validate_cone(br,tr,h,t.modeling)?;let mut o=std::ptr::null_mut();let s=unsafe{umlcad_occt_cone(br,tr,h,&mut o)};if s!=OCCT_OK{return Err(Self::map_status(s,"OCCT cone construction failed"));}let raw=NonNull::new(o).ok_or(GeometryError::Unsupported("OCCT returned a null shape"))?;Ok(GeometryResult{shape:OcctShape{raw},kind:GeometryKind::Solid,evidence:self.evidence(GeometryStatus::Success,t,None)})}
...
mod tests {
    ...
    #[test] fn cone_is_valid_and_canonical(){let b=OcctBackend::new();let s=b.cone_solid(10.,5.,20.,T).unwrap();assert_eq!(s.kind,GeometryKind::Solid);assert_eq!(b.validate(&s.shape,T).unwrap(),ValidationResult{valid:true,manifold:true,message:None});assert_eq!(b.topology_counts(&s.shape,T).unwrap(),TopologyCounts{solids:1,shells:1,faces:3,edges:3,vertices:2});}
    #[test] fn cone_rejects_degenerate_inputs(){let b=OcctBackend::new();for x in [(0.,5.,20.),(10.,0.,20.),(10.,5.,0.),(-1.,5.,20.),(10.,-1.,20.),(10.,5.,-1.),(f64::NAN,5.,20.),(10.,f64::INFINITY,20.)]{assert!(matches!(b.cone_solid(x.0,x.1,x.2,T),Err(GeometryError::InvalidInput(_))));}}
    #[test] fn cone_bounds_match_axis_aligned_geometry(){let b=OcctBackend::new();let bounds=b.bounding_box(&b.cone_solid(10.,5.,20.,T).unwrap().shape,T).unwrap();assert!((bounds.min_x+10.).abs()<=1e-9);assert!((bounds.max_x-10.).abs()<=1e-9);assert!((bounds.min_y+10.).abs()<=1e-9);assert!((bounds.max_y-10.).abs()<=1e-9);assert!((bounds.min_z-0.).abs()<=1e-9);assert!((bounds.max_z-20.).abs()<=1e-9);}
