...
impl NurbsSurfaceBackend for OcctBackend {
    fn nurbs_surface3d(&self, definition: &NurbsSurface3DDefinition, tolerance: ToleranceContext) -> Result<GeometryResult<Self::Shape>, GeometryError> {
        tolerance.validate()?;
        definition.validate().map_err(|_| GeometryError::InvalidInput("invalid NURBS surface definition"))?;
        let mut poles_xyz = Vec::with_capacity(definition.control_points.len() * 3);
        for point in &definition.control_points { poles_xyz.extend([point.x, point.y, point.z]); }
        let mut raw = std::ptr::null_mut();
        let status = unsafe { umlcad_occt_nurbs_surface3d(poles_xyz.as_ptr(), definition.count_u as u32, definition.count_v as u32, definition.weights.as_ptr(), definition.knots_u.as_ptr(), definition.knots_u.len() as u32, definition.knots_v.as_ptr(), definition.knots_v.len() as u32, definition.degree_u as u32, definition.degree_v as u32, tolerance.modeling, &mut raw) };
        if status != OCCT_OK { return Err(Self::status(status, "OCCT NURBS surface construction failed")); }
        let raw = NonNull::new(raw).ok_or(GeometryError::Unsupported("OCCT returned null"))?;
        Ok(GeometryResult { shape: OcctShape::from_raw(raw, GeometryKind::Surface), kind: GeometryKind::Surface, evidence: GeometryEvidence { status: GeometryStatus::Success, backend: self.backend_name(), tolerance, message: None } })
    }
}
...
impl NurbsSurfaceDifferentialBackend for OcctBackend {
    fn nurbs_surface3d_differential_at(&self, shape: &Self::Shape, u: f64, v: f64, tolerance: ToleranceContext) -> Result<NurbsSurfaceDifferential, GeometryError> {
        tolerance.validate()?;
        if shape.kind != GeometryKind::Surface { return Err(GeometryError::InvalidInput("NURBS differential requires a surface shape")); }
        if !u.is_finite() || !v.is_finite() { return Err(GeometryError::InvalidInput("NURBS parameters must be finite")); }
        let mut values = [0.0f64; 18];
        let status = unsafe { umlcad_occt_nurbs_surface3d_differential(shape.raw.as_ptr(), u, v, values.as_mut_ptr()) };
