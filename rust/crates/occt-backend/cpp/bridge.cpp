...
extern "C" int32_t umlcad_occt_shape_edge_descriptor_count(const umlcad_occt_shape* input,uint32_t* out_count){if(input==nullptr||out_count==nullptr)return UMLCAD_OCCT_INVALID_ARGUMENT;*out_count=0;try{if(input->value.IsNull())return UMLCAD_OCCT_NULL_SHAPE;for(TopExp_Explorer explorer(input->value,TopAbs_EDGE);explorer.More();explorer.Next())++*out_count;return UMLCAD_OCCT_OK;}catch(...){return UMLCAD_OCCT_INTERNAL_ERROR;}}
extern "C" int32_t umlcad_occt_shape_edge_descriptors(const umlcad_occt_shape* input,double* out_values,uint32_t capacity){if(input==nullptr||out_values==nullptr)return UMLCAD_OCCT_INVALID_ARGUMENT;try{if(input->value.IsNull())return UMLCAD_OCCT_NULL_SHAPE;uint32_t index=0;for(TopExp_Explorer explorer(input->value,TopAbs_EDGE);explorer.More();explorer.Next()){if(index>=capacity)return UMLCAD_OCCT_INVALID_ARGUMENT;if(!edgeDescriptor(input->value,explorer.Current(),out_values+(static_cast<size_t>(index)*9U)))return UMLCAD_OCCT_INTERNAL_ERROR;++index;}return UMLCAD_OCCT_OK;}catch(...){return UMLCAD_OCCT_INTERNAL_ERROR;}}
