...
bool faceDescriptor(const TopoDS_Shape& face, double* values) {
    ...
    if (!isFiniteValue(min_x) || !isFiniteValue(min_y) || !isFiniteValue(min_z)
        || !isFiniteValue(max_x) || !isFiniteValue(max_y) || !isFiniteValue(max_z)) return false;
...
extern "C" int32_t umlcad_occt_shape_bounding_box(const umlcad_occt_shape* input,double* out_bounds){if(input==nullptr||out_bounds==nullptr)return UMLCAD_OCCT_INVALID_ARGUMENT;for(int index=0;index<6;++index)out_bounds[index]=0.0;try{if(input->value.IsNull())return UMLCAD_OCCT_NULL_SHAPE;Bnd_Box box;box.SetGap(0.0);BRepBndLib::AddOptimal(input->value,box,Standard_False,Standard_False);if(box.IsVoid())return UMLCAD_OCCT_TRANSFORM_FAILED;double min_x,min_y,min_z,max_x,max_y,max_z;box.Get(min_x,min_y,min_z,max_x,max_y,max_z);if(!isFiniteValue(min_x)||!isFiniteValue(min_y)||!isFiniteValue(min_z)||!isFiniteValue(max_x)||!isFiniteValue(max_y)||!isFiniteValue(max_z))return UMLCAD_OCCT_INTERNAL_ERROR;out_bounds[0]=min_x;out_bounds[1]=min_y;out_bounds[2]=min_z;out_bounds[3]=max_x;out_bounds[4]=max_y;out_bounds[5]=max_z;return UMLCAD_OCCT_OK;}catch(...){return UMLCAD_OCCT_INTERNAL_ERROR;}}
