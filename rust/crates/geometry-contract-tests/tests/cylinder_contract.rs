...
fn cylinder_contract_is_analytic_solid() {
    ...
    assert_eq!(
        backend.validate(&result.shape, TOLERANCE).unwrap(),
        ValidationResult {
            valid: true,
            manifold: true,
            message: None,
...
fn cylinder_contract_exposes_canonical_topology() {
    ...
    assert_eq!(
        backend.topology_counts(&shape, TOLERANCE).unwrap(),
        TopologyCounts {
            solids: 1,
            shells: 1,
            faces: 3,
...
fn cylinder_contract_survives_immutable_transforms() {
    ...
    assert_eq!(
        backend.validate(&source, TOLERANCE).unwrap(),
        ValidationResult { valid: true, manifold: true, message: None }
    );
    assert_eq!(
        backend.validate(&translated, TOLERANCE).unwrap(),
        ValidationResult { valid: true, manifold: true, message: None }
    );
    assert_eq!(
        backend.validate(&rotated, TOLERANCE).unwrap(),
        ValidationResult { valid: true, manifold: true, message: None }
    );
}
