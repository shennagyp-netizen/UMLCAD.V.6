...
fn assert_bounds(
    backend: &OcctBackend,
    shape: &<OcctBackend as GeometryBackend>::Shape,
    expected: [f64; 6],
) {
...
fn translation_matches_independent_affine_bound_transform() {
    ...
    assert_bounds(
        &backend,
        &translated,
        [100.0, -50.0, 7.5, 110.0, -30.0, 37.5],
    );
...
fn two_translations_equal_one_composed_translation() {
    ...
    assert_close(twice_box.min_x, once_box.min_x, T.validation, "min_x");
    assert_close(twice_box.min_y, once_box.min_y, T.validation, "min_y");
    assert_close(twice_box.min_z, once_box.min_z, T.validation, "min_z");
    assert_close(twice_box.max_x, once_box.max_x, T.validation, "max_x");
    assert_close(twice_box.max_y, once_box.max_y, T.validation, "max_y");
    assert_close(twice_box.max_z, once_box.max_z, T.validation, "max_z");
}
