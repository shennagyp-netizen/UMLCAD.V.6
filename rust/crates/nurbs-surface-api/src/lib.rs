...
fn require_second_order_continuity(t: f64, degree: usize, knots: &[f64], count: usize) -> Result<(), NurbsSurfaceEvaluationError> { if t != knots[degree] && t != knots[count] && knot_continuity_order(t, degree, knots, count) < 2 { return Err(NurbsSurfaceEvaluationError::InsufficientContinuity); } Ok(()) }
