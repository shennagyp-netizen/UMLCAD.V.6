...
impl NurbsSurfaceDifferential {
    pub fn normal(&self) -> Result<Point3, NurbsSurfaceEvaluationError> {
        ...
        if !magnitude.is_finite() {
            return Err(NurbsSurfaceEvaluationError::Overflow);
        }
        if magnitude == 0.0 {
            return Err(NurbsSurfaceEvaluationError::DegenerateNormal);
        }
...
impl NurbsSurface3DDefinition {
    ...
    pub fn differential_at(
        &self,
        u: f64,
        v: f64,
    ) -> Result<NurbsSurfaceDifferential, NurbsSurfaceEvaluationError> {
        self.validate().map_err(|_| NurbsSurfaceEvaluationError::Overflow)?;
        if !u.is_finite() || !v.is_finite() {
            return Err(NurbsSurfaceEvaluationError::NonFinite);
        }
        let ((u0, u1), (v0, v1)) = self.parameter_domain().map_err(|_| NurbsSurfaceEvaluationError::Overflow)?;
        if u < u0 || u > u1 || v < v0 || v > v1 {
            return Err(NurbsSurfaceEvaluationError::OutOfDomain);
        }
        ...
        for i in 0..self.count_u {
            for j in 0..self.count_v {
                ...
                for ku in 0..=2 {
                    for kv in 0..=2 {
                        let coefficient = bu[i][ku] * bv[j][kv];
                        homogeneous[ku][kv] = homogeneous[ku][kv].add(h.scale(coefficient));
                    }
                }
            }
        }
...
fn knot_continuity_order(parameter: f64, degree: usize, knots: &[f64], count: usize) -> usize {
    ...
    if parameter == knots[degree] || parameter == knots[count] {
        degree
    } else {
        degree.saturating_sub(multiplicity)
    }
}

fn require_second_order_continuity(
    parameter: f64,
    degree: usize,
    knots: &[f64],
    count: usize,
) -> Result<(), NurbsSurfaceEvaluationError> {
    if parameter == knots[degree] || parameter == knots[count] {
        return Ok(());
    }
    if knot_continuity_order(parameter, degree, knots, count) < 2 {
        return Err(NurbsSurfaceEvaluationError::InsufficientContinuity);
    }
    Ok(())
}

fn basis_span(t: f64, degree: usize, knots: &[f64], count: usize) -> usize {
    if t >= knots[count] {
        return count - 1;
    }
    if t <= knots[degree] {
...
fn basis_derivatives(t: f64, degree: usize, knots: &[f64], count: usize) -> Vec<[f64; 3]> {
    let span = basis_span(t, degree, knots, count);
    let mut values = vec![[0.0; 3]; count];
    for i in 0..count {
        for order in 0..=2 {
            values[i][order] = basis_derivative_recursive(i, degree, t, knots, order);
        }
    }
    let _ = span;
    values
...
fn basis_derivative_recursive(i: usize, degree: usize, t: f64, knots: &[f64], order: usize) -> f64 {
    ...
    if order == 0 {
        let left = if knots[i + degree] == knots[i] { 0.0 } else { (t - knots[i]) / (knots[i + degree] - knots[i]) * basis_derivative_recursive(i, degree - 1, t, knots, 0) };
        let right = if knots[i + degree + 1] == knots[i + 1] { 0.0 } else { (knots[i + degree + 1] - t) / (knots[i + degree + 1] - knots[i + 1]) * basis_derivative_recursive(i + 1, degree - 1, t, knots, 0) };
        return left + right;
    }
...
fn rationalize_differential(
    h: [[Homogeneous; 3]; 3],
) -> Result<NurbsSurfaceDifferential, NurbsSurfaceEvaluationError> {
    if h.iter().flatten().any(|x| !x.xw.is_finite() || !x.yw.is_finite() || !x.zw.is_finite() || !x.w.is_finite()) {
        return Err(NurbsSurfaceEvaluationError::Overflow);
    }
    let w = h[0][0].w;
    if w <= 0.0 || !w.is_finite() {
        return Err(NurbsSurfaceEvaluationError::ZeroProjectiveWeight);
    }
    ...
    let puu = quotient_second(h[2][0], h[1][0], h[0][0], pu, point, 2.0)?;
    let puv = quotient_mixed(h[1][1], h[1][0], h[0][1], h[0][0], pu, pv, point)?;
    let pvv = quotient_second(h[0][2], h[0][1], h[0][0], pv, point, 2.0)?;

    if [point, pu, pv, puu, puv, pvv].iter().flat_map(|p| [p.x, p.y, p.z]).any(|x| !x.is_finite()) {
        return Err(NurbsSurfaceEvaluationError::Overflow);
    }
    Ok(NurbsSurfaceDifferential { point, du: pu, dv: pv, duu: puu, duv: puv, dvv: pvv })
}

fn homogeneous_to_point(h: Homogeneous, w: f64) -> Point3 {
    Point3 { x: h.xw / w, y: h.yw / w, z: h.zw / w }
}

fn quotient_first(d: Homogeneous, base: Homogeneous, point: Point3) -> Result<Point3, NurbsSurfaceEvaluationError> {
    Ok(Point3 {
        x: d.xw / base.w - point.x * d.w / base.w,
        y: d.yw / base.w - point.y * d.w / base.w,
        z: d.zw / base.w - point.z * d.w / base.w,
    })
}

fn quotient_second(d2: Homogeneous, d1: Homogeneous, base: Homogeneous, p1: Point3, p: Point3, factor: f64) -> Result<Point3, NurbsSurfaceEvaluationError> {
    let w = base.w;
    let w1 = d1.w;
    let w2 = d2.w;
    Ok(Point3 {
        x: (d2.xw - factor * w1 * p1.x - w2 * p.x) / w,
        y: (d2.yw - factor * w1 * p1.y - w2 * p.y) / w,
        z: (d2.zw - factor * w1 * p1.z - w2 * p.z) / w,
    })
}
...
fn quotient_mixed(
    ...
    dv: Point3,
    p: Point3,
) -> Result<Point3, NurbsSurfaceEvaluationError> {
    let w = base.w;
    Ok(Point3 {
        x: (d2.xw - du_h.w * dv.x - dv_h.w * du.x - d2.w * p.x) / w,
        y: (d2.yw - du_h.w * dv.y - dv_h.w * du.y - d2.w * p.y) / w,
        z: (d2.zw - du_h.w * dv.z - dv_h.w * du.z - d2.w * p.z) / w,
    })
}
...
mod tests {
    ...
    #[test]
    fn bilinear_first_derivatives_are_constant() {
    ...
    #[test]
    fn bilinear_normal_is_unit() {
        let d = bilinear().differential_at(0.25, 0.75).unwrap();
        let n = d.normal().unwrap();
        assert!((n.norm() - 1.0).abs() < 1e-14);
        assert!(n.dot(d.du).abs() < 1e-14);
        assert!(n.dot(d.dv).abs() < 1e-14);
    }

    #[test]
    fn out_of_domain_is_rejected() {
        assert_eq!(bilinear().differential_at(-0.1, 0.5), Err(NurbsSurfaceEvaluationError::OutOfDomain));
    }
    ...
    fn invalid_weight_is_rejected() {
        let mut s = bilinear();
        s.weights[2] = 0.0;
        assert!(s.differential_at(0.5, 0.5).is_err());
    }
    ...
    #[test]
    fn unused_helper_is_not_required() {
        let _ = homogeneous_to_point;
    }
}
