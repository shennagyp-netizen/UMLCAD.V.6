...
impl NurbsSurface3DDefinition {
    pub fn new(degree_u: usize, degree_v: usize, control_points: Vec<Point3>, weights: Vec<f64>, count_u: usize, count_v: usize, knots_u: Vec<f64>, knots_v: Vec<f64>) -> Self {
        Self { degree_u, degree_v, control_points, weights, count_u, count_v, knots_u, knots_v }
    }
    ...
    pub fn validate(&self) -> Result<(), NurbsSurfaceDefinitionError> {
        ...
        let u0 = self.knots_u[self.degree_u]; let u1 = self.knots_u[self.count_u];
        let v0 = self.knots_v[self.degree_v]; let v1 = self.knots_v[self.count_v];
    ...
    pub fn parameter_domain(&self) -> Result<((f64, f64), (f64, f64)), NurbsSurfaceDefinitionError> {
        self.validate()?;
...
mod tests {
    ...
    fn bilinear() -> NurbsSurface3DDefinition {
        NurbsSurface3DDefinition::new(1, 1,
            vec![
    ...
    #[test]
    fn bilinear_surface_is_valid() { assert_eq!(bilinear().parameter_domain().unwrap(), ((0.0, 1.0), (0.0, 1.0))); }

    #[test]
    fn invalid_weight_is_rejected() { let mut s = bilinear(); s.weights[2] = 0.0; assert_eq!(s.validate(), Err(NurbsSurfaceDefinitionError::InvalidWeight)); }

    #[test]
    fn nondecreasing_knot_contract_is_enforced() { let mut s = bilinear(); s.knots_v[2] = -1.0; assert_eq!(s.validate(), Err(NurbsSurfaceDefinitionError::KnotsMustBeNondecreasing)); }
}
