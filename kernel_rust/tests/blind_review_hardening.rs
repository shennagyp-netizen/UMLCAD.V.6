...
fn snapshot_with_geometry(geometry: Vec<(&str, Geometry)>) -> SemanticSnapshot {
    SemanticSnapshot {
        ...
        geometry: geometry
            .into_iter()
            .map(|(id, geometry)| kernel_rust::GeometryItem {
                id: id.into(),
                geometry,
...
fn symmetric_relation_has_only_independent_residuals() {
    let snapshot = snapshot_with_geometry(vec![
        ("a", Geometry::Line(Line { start: Point { x: -2.0, y: 1.0 }, end: Point { x: -2.0, y: 3.0 } })),
        ("b", Geometry::Line(Line { start: Point { x: 2.0, y: 1.0 }, end: Point { x: 2.0, y: 3.0 } })),
        ("o", Geometry::Line(Line { start: Point { x: 0.0, y: 2.0 }, end: Point { x: 1.0, y: 2.0 } })),
    ]);
    let relation = Relation::Symmetric {
        first: RelationPoint::Endpoint { geometry_id: "a".into(), point: kernel_rust::functions::snapshot::Endpoint::Start },
        second: RelationPoint::Endpoint { geometry_id: "b".into(), point: kernel_rust::functions::snapshot::Endpoint::Start },
        about: RelationPoint::Endpoint { geometry_id: "o".into(), point: kernel_rust::functions::snapshot::Endpoint::Start },
    };
...
fn line_arc_detects_interior_intersection() {
    ...
    let arc = Geometry::Arc(Arc { center: Point { x: 0.0, y: 0.0 }, radius: 10.0, start_angle: 0.0, end_angle: PI });
...
fn arc_arc_detects_intersection_not_at_endpoints_or_midpoints() {
    let a = Geometry::Arc(Arc { center: Point { x: 0.0, y: 0.0 }, radius: 5.0, start_angle: 0.0, end_angle: PI / 2.0 });
...
fn multi_wrap_arc_is_length_and_set_membership_consistent() {
    let arc = Arc { center: Point { x: 0.0, y: 0.0 }, radius: 2.0, start_angle: 7.0 * PI, end_angle: 10.0 * PI };
...
fn multi_wrap_arc_spatial_membership_is_offset_invariant() {
    let arc = Geometry::Arc(Arc { center: Point { x: 0.0, y: 0.0 }, radius: 2.0, start_angle: 7.0 * PI, end_angle: 10.0 * PI });
...
fn self_loop_contributes_two_to_topology_degree() {
    let snapshot = snapshot_with_geometry(vec![
        ("loop", Geometry::Arc(Arc { center: Point { x: 0.0, y: 0.0 }, radius: 1.0, start_angle: 0.0, end_angle: 2.0 * PI })),
        ("tail", Geometry::Line(Line { start: Point { x: 1.0, y: 0.0 }, end: Point { x: 2.0, y: 0.0 } })),
    ]);
...
fn rank_detection_is_invariant_to_uniform_column_scaling() {
    let small = scaled_damped_qr(&[vec![1.0e-12, 0.0], vec![0.0, 1.0e-12]], &[1.0, 1.0], 0.0, 1.0e-10).unwrap();
    let large = scaled_damped_qr(&[vec![1.0e6, 0.0], vec![0.0, 1.0e6]], &[1.0, 1.0], 0.0, 1.0e-10).unwrap();
