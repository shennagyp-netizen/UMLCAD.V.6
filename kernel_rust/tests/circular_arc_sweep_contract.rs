...
fn circular_arc_sweep_surface_parameterization_matches_torus_geometry() {
    let sweep = quarter_tube();
    let q = sweep.surface_point_at(core::f64::consts::FRAC_PI_4, 0.0).unwrap();
    let expected = 12.0 / core::f64::consts::SQRT_2;
...
#[test]
fn circular_arc_sweep_rejects_nonfinite_surface_parameters() {
    let sweep = quarter_tube();
    assert_eq!(sweep.surface_point_at(f64::NAN, 0.0), Err(SweepArcError::NonFinite));
    assert_eq!(sweep.surface_point_at(0.0, f64::INFINITY), Err(SweepArcError::NonFinite));
}
