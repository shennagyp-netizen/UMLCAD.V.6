...
fn rational_curve_reduces_to_bspline_when_all_weights_are_one() {
    ...
    let ordinary = bspline::BSplineCurve2D::new(
        2,
        vec![bspline::Point2 { x: 0.0, y: 0.0 }, bspline::Point2 { x: 1.0, y: 2.0 }, bspline::Point2 { x: 3.0, y: 0.0 }],
...
fn validation_rejects_invalid_weights_and_structure() {
    ...
    assert_eq!(NurbsCurve2D::new(2, points.clone(), vec![1.0, 1.0, 1.0], vec![0.0, 0.0, 1.0]).validate(), Err(NurbsError::InvalidKnotCount));
