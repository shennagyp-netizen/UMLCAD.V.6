...
fn positive_weights_keep_points_inside_the_control_hull() {
    let curve = NurbsCurve2D::new(
        2,
        vec![p(-10.0, -3.0), p(4.0, 20.0), p(30.0, 5.0), p(50.0, -4.0)],
        vec![0.5, 2.0, 1.5, 3.0],
...
fn validation_rejects_invalid_weights_and_structure() {
    ...
    assert_eq!(
        NurbsCurve2D::new(2, points.clone(), vec![1.0, 0.0, 1.0], knots.clone()).validate(),
        Err(NurbsError::InvalidWeight)
    );
    assert_eq!(
        NurbsCurve2D::new(2, points.clone(), vec![1.0, -1.0, 1.0], knots.clone()).validate(),
        Err(NurbsError::InvalidWeight)
    );
    assert!(
        NurbsCurve2D::new(2, points.clone(), vec![1.0, f64::NAN, 1.0], knots.clone())
            .validate()
            .is_err()
    );
    assert_eq!(
        NurbsCurve2D::new(2, points.clone(), vec![1.0, 1.0], knots.clone()).validate(),
        Err(NurbsError::InvalidWeightCount)
    );
    assert_eq!(
        NurbsCurve2D::new(2, points.clone(), vec![1.0, 1.0, 1.0], vec![0.0, 0.0, 1.0]).validate(),
        Err(NurbsError::InvalidKnotCount)
    );
    assert_eq!(
        NurbsCurve2D::new(2, points, vec![1.0, 1.0, 1.0], vec![0.0, 0.0, 0.5, 0.25, 1.0, 1.0]).validate(),
        Err(NurbsError::KnotsMustBeNondecreasing)
    );
}
