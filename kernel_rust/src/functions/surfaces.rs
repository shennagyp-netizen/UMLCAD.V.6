...
impl PlanarSurface {
    ...
    pub fn distance_to_point(&self, point: Point3) -> Result<f64, SurfaceError> {
        ...
        let xmin = self.center.x - self.width * 0.5;
        let xmax = self.center.x + self.width * 0.5;
        let ymin = self.center.y - self.depth * 0.5;
        let ymax = self.center.y + self.depth * 0.5;
        let dx = if point.x < xmin {
            xmin - point.x
    ...
    pub fn translated(&self, dx: f64, dy: f64, dz: f64) -> Result<Self, SurfaceError> {
        self.validate()?;
        validate_finite_delta(dx, dy, dz)?;
        let center = Point3 {
            x: self.center.x + dx,
            y: self.center.y + dy,
            z: self.center.z + dz,
...
pub struct SphereSurface {
    ...
    pub radius_value: f64,
}

impl SphereSurface {
    pub fn new(center: Point3, radius: f64) -> Self {
        Self { center_point: center, radius_value: radius }
    }
    ...
    pub fn validate(&self) -> Result<(), SurfaceError> {
        if !self.center_point.x.is_finite()
            || !self.center_point.y.is_finite()
            || !self.center_point.z.is_finite()
            || !self.radius_value.is_finite()
        {
            return Err(SurfaceError::NonFinite);
        }
        if self.radius_value <= 0.0 {
            return Err(SurfaceError::InvalidRadius);
        }
        if !self.area().is_finite() {
            return Err(SurfaceError::NonFinite);
        }
        ...
        let values = [
            self.center_point.x - r,
            self.center_point.x + r,
            self.center_point.y - r,
            self.center_point.y + r,
            self.center_point.z - r,
            self.center_point.z + r,
        ];
        if values.iter().any(|value| !value.is_finite()) {
            return Err(SurfaceError::NonFinite);
    ...
    pub fn area(&self) -> f64 {
        4.0 * PI * self.radius_value * self.radius_value
    }
    ...
    pub fn normal_at(&self, u: f64, v: f64) -> Result<Point3, SurfaceError> {
        let radial = self.center_point.vector_to(self.point_at(u, v)?);
    ...
    pub fn bounding_box(&self) -> Result<BoundingBox3, SurfaceError> {
        ...
        let r = self.radius_value;
        Ok(BoundingBox3 {
            min: Point3 { x: self.center_point.x - r, y: self.center_point.y - r, z: self.center_point.z - r },
            max: Point3 { x: self.center_point.x + r, y: self.center_point.y + r, z: self.center_point.z + r },
        })
    }
    ...
    pub fn distance_to_point(&self, point: Point3) -> Result<f64, SurfaceError> {
        self.validate()?;
        validate_point(point)?;
        let d = self.center_point.distance(point);
        if !d.is_finite() {
            return Err(SurfaceError::NonFinite);
        }
    ...
    pub fn translated(&self, dx: f64, dy: f64, dz: f64) -> Result<Self, SurfaceError> {
        ...
        validate_finite_delta(dx, dy, dz)?;
        let center = Point3 {
            x: self.center_point.x + dx,
            y: self.center_point.y + dy,
            z: self.center_point.z + dz,
        };
        validate_point(center)?;
