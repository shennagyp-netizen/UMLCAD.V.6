...
impl CircularSweep {
    pub fn new(path_start: Point3, path_end: Point3, radius: f64) -> Self { Self { path_start, path_end, radius } }
    fn axis_and_length(&self) -> Result<(Point3, f64), SweepError> {
        if !finite_point(self.path_start) || !finite_point(self.path_end) || !self.radius.is_finite() { return Err(SweepError::NonFinite); }
    ...
    fn frame(&self) -> Result<(Point3, Point3, Point3), SweepError> {
        let (axis, _) = self.axis_and_length()?;
        let candidates = [Point3 { x: 1.0, y: 0.0, z: 0.0 }, Point3 { x: 0.0, y: 1.0, z: 0.0 }, Point3 { x: 0.0, y: 0.0, z: 1.0 }];
...
impl CircularArcSweep {
    pub fn new(center: Point3, path_radius: f64, profile_radius: f64, start_angle: f64, end_angle: f64) -> Self { Self { center, path_radius, profile_radius, start_angle, end_angle } }
    ...
    pub fn surface_point_at(&self, path_parameter: f64, profile_angle: f64) -> Result<Point3, SweepArcError> {
        ...
        let point = Point3 { x: self.center.x + radial * theta.cos(), y: self.center.y + radial * theta.sin(), z: self.center.z + self.profile_radius * profile_angle.sin() };
    ...
    pub fn bounding_box(&self) -> Result<BoundingBox3, SweepArcError> {
        ...
        for theta in angles {
            for radial in [self.path_radius - self.profile_radius, self.path_radius + self.profile_radius] {
                let point = Point3 { x: self.center.x + radial * theta.cos(), y: self.center.y + radial * theta.sin(), z: self.center.z };
                min.x = min.x.min(point.x); min.y = min.y.min(point.y); max.x = max.x.max(point.x); max.y = max.y.max(point.y);
            }
        }
