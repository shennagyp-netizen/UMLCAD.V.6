...
fn segment_distance(a: Line, b: Line) -> f64 {
    ...
    [b.start.distance(closest(a, b.start)), b.end.distance(closest(a, b.end)), a.start.distance(closest(b, a.start)), a.end.distance(closest(b, a.end))].into_iter().fold(f64::INFINITY, f64::min)
}
...
fn line_circle(l: Line, c: Circle) -> f64 { (c.center.distance(closest(l, c.center)) - c.radius).max(0.0) }
fn point_on_arc(a: Arc, p: Point) -> bool { a.contains_point(p) }
fn push_unique(points: &mut Vec<Point>, p: Point) { if !points.iter().any(|q| q.distance(p) <= PARAM_EPSILON) { points.push(p); } }

fn line_circle_intersections(l: Line, c: Circle) -> Vec<Point> {
    let d = l.end.sub(l.start);
    let f = l.start.sub(c.center);
    let aa = d.dot(d);
    if aa <= EPSILON { return Vec::new(); }
    let bb = 2.0 * f.dot(d);
    let cc = f.dot(f) - c.radius * c.radius;
    let disc = bb * bb - 4.0 * aa * cc;
    if disc < -PARAM_EPSILON { return Vec::new(); }
    if disc.abs() <= PARAM_EPSILON {
        let t = -bb / (2.0 * aa);
        if (-PARAM_EPSILON..=1.0 + PARAM_EPSILON).contains(&t) { return vec![l.start.add(d.scale(t.clamp(0.0, 1.0)))]; }
        return Vec::new();
    }
    let root = disc.max(0.0).sqrt();
    let mut out = Vec::new();
    for t in [(-bb - root) / (2.0 * aa), (-bb + root) / (2.0 * aa)] {
        if (-PARAM_EPSILON..=1.0 + PARAM_EPSILON).contains(&t) { push_unique(&mut out, l.start.add(d.scale(t.clamp(0.0, 1.0)))); }
    }
    out
}
...
fn circle_circle_intersections(a: Circle, b: Circle) -> Vec<Point> {
    ...
    let d = delta.norm();
    if d <= EPSILON || d > a.radius + b.radius + PARAM_EPSILON || d + PARAM_EPSILON < (a.radius - b.radius).abs() { return Vec::new(); }
    let x = (a.radius * a.radius - b.radius * b.radius + d * d) / (2.0 * d);
    let h2 = a.radius * a.radius - x * x;
    if h2 < -PARAM_EPSILON { return Vec::new(); }
    ...
    if h > PARAM_EPSILON { push_unique(&mut out, base.sub(perp.scale(h))); }
    out
}
...
fn arc_extreme_candidates(a: Arc, toward: Point) -> Vec<Point> {
    ...
    let n = v.norm();
    if n <= EPSILON { return Vec::new(); }
    let q = a.center.add(v.scale(a.radius / n));
    let q2 = a.center.sub(v.scale(a.radius / n));
    let mut out = Vec::new();
    if point_on_arc(a, q) { push_unique(&mut out, q); }
    if point_on_arc(a, q2) { push_unique(&mut out, q2); }
    out
}

fn line_arc(l: Line, a: Arc) -> f64 {
    let circle = Circle { center: a.center, radius: a.radius };
    for p in line_circle_intersections(l, circle) { if point_on_arc(a, p) { return 0.0; } }
    let mut best = f64::INFINITY;
    for p in [l.start, l.end] { best = best.min(a.distance_to_point(p)); }
    for p in [a.start_point(), a.end_point()] { best = best.min(l.start.distance(p).min(l.end.distance(p))); }
    let q = closest(l, a.center);
    let radial = q.sub(a.center).norm();
    if radial > EPSILON {
        let p = a.center.add(q.sub(a.center).scale(a.radius / radial));
        if point_on_arc(a, p) { best = best.min(p.distance(q)); }
    }
    best
}

fn arc_circle(a: Arc, c: Circle) -> f64 {
    for p in circle_circle_intersections(Circle { center: a.center, radius: a.radius }, c) { if point_on_arc(a, p) { return 0.0; } }
    let mut best = f64::INFINITY;
    for p in [a.start_point(), a.end_point()] { best = best.min((p.distance(c.center) - c.radius).abs()); }
    for p in arc_extreme_candidates(a, c.center) { best = best.min((p.distance(c.center) - c.radius).abs()); }
    if a.center.distance(c.center) <= EPSILON { best.min((a.radius - c.radius).abs()) } else { best }
}

fn concentric_arc_overlap(a: Arc, b: Arc) -> bool {
    a.contains_angle(b.start_angle) || a.contains_angle(b.end_angle) || b.contains_angle(a.start_angle) || b.contains_angle(a.end_angle)
}
fn arc_arc(a: Arc, b: Arc) -> f64 {
    let ca = Circle { center: a.center, radius: a.radius };
    let cb = Circle { center: b.center, radius: b.radius };
    for p in circle_circle_intersections(ca, cb) { if point_on_arc(a, p) && point_on_arc(b, p) { return 0.0; } }
    let mut best = f64::INFINITY;
    for p in [a.start_point(), a.end_point()] { best = best.min(b.distance_to_point(p)); }
    for p in [b.start_point(), b.end_point()] { best = best.min(a.distance_to_point(p)); }
    for p in arc_extreme_candidates(a, b.center) { best = best.min(b.distance_to_point(p)); }
    for p in arc_extreme_candidates(b, a.center) { best = best.min(a.distance_to_point(p)); }
    if a.center.distance(b.center) <= EPSILON && concentric_arc_overlap(a, b) { best.min((a.radius - b.radius).abs()) } else { best }
}
